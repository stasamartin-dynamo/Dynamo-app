...
.cr{display:flex;align-items:flex-start;justify-content:space-between;gap:8}
...
export default function App() {
  ...
  const pgHome=()=>{
    ...
    {/* Příští zápas */}
    {!isV&&!isTr&&(<div style={{marginBottom:10}}><div className="lb" style={{display:'flex',alignItems:'center',gap:5}}>🏟️ Příští zápas</div>{nxM?<div style={ecs} onClick={()=>{go("matches");setTimeout(()=>setSelM({id:nxM.id}),50)}}><div className="cr"><div style={{flex:1,minWidth:0}}><div className="ctt" style={{fontSize:14,fontWeight:700}}><VS op={nxM.opponent} sz={14} home={nxM.location==="Domácí"}/></div><div className="css2" style={{marginTop:3}}>{fd(nxM.date)} · {nxM.time}</div></div><span className={`tg ${nxM.location==="Domácí"?"th":"ta"}`} style={{fontSize:10,padding:'4px 10px',flexShrink:0}}>{nxM.location==="Domácí"?"Doma":"Venku"}</span></div></div>:<div style={{...ecs,cursor:'default',opacity:.5,textAlign:'center',padding:16}}><div style={{fontSize:11,color:'var(--t3)'}}>Žádný nadcházející zápas</div></div>}</div>)}
  ...
  const VS=({op,sz,home})=>{const s=sz||12;return <span style={{fontSize:s,fontWeight:700,lineHeight:1.3}}>{home===false?<>{op} <span style={{fontStyle:'italic',fontSize:s*.8,color:'var(--t3)',fontWeight:400}}>vs</span> DD</>:<>DD <span style={{fontStyle:'italic',fontSize:s*.8,color:'var(--t3)',fontWeight:400}}>vs</span> {op}</>}</span>};
  ...
  const pgMatches=()=>{const selMatch=selM?T.matches.find(x=>x.id===(selM.id||selM)):null;if(selMatch){const m=selMatch; return (<div>
    ...
    {upcoming.map(m=> <div className="c2" key={m.id} onClick={()=>setSelM({id:m.id})}><div className="cr"><div style={{flex:1,minWidth:0}}><div className="ctt"><VS op={m.opponent} home={m.location==="Domácí"}/></div><div className="css2">{fd(m.date)} · {m.time}{m.meetTime&&" · sraz "+m.meetTime}</div></div><div style={{display:'flex',gap:5,flexShrink:0,alignItems:'center'}}><span className={`tg ${m.location==="Domácí"?"th":"ta"}`}>{m.location==="Domácí"?"Doma":"Venku"}</span><button className="ib d" onClick={e=>{e.stopPropagation();del("matches",m.id)}}><Ic.Del/></button></div></div><div style={{fontSize:10,color:'var(--t3)',marginTop:4}}>✓ {names.filter(n=>(m.attendance||{})[n]).length} · ✗ {names.filter(n=>(m.excuses||{})[n]).length} · ? {names.length-names.filter(n=>(m.attendance||{})[n]).length-names.filter(n=>(m.excuses||{})[n]).length}</div></div>)}
    {past.length>0&&<div className="lb" style={{marginTop:12}}>Proběhlo</div>}
    {past.map(m=> <div className="c2" key={m.id} onClick={()=>setSelM({id:m.id})} style={{opacity:.85}}><div className="cr"><div style={{flex:1,minWidth:0}}><div className="ctt" style={{}}><span style={{color:'#16a34a',fontWeight:700,marginRight:4}}>✓</span><VS op={m.opponent} home={m.location==="Domácí"}/></div><div className="css2">{fd(m.date)} · {m.result||m.time}</div></div><span style={{background:'#16a34a',color:'#fff',padding:'3px 10px',borderRadius:12,fontSize:11,fontWeight:700,flexShrink:0}}>✓ Hotovo</span></div></div>)}</div>)};
