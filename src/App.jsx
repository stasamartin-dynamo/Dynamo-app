...
export default function App() {
  ...
  const modals=()=>{if(!mod) return null;
    ...
    if(mod==="aPh") return (<FM title="Přidat fotku" onS={f=>{const n=nt("Nová fotka","photos");saveT({...T,...n,photos:[...T.photos,{id:"ph_"+uid(),url:f.get('u'),caption:f.get('c'),date:td()}]});setMod(null)}} ch={<>{F("u","URL","url","https://...")}{F("c","Popis","text","",false)}<button type="submit" className="fs">Přidat</button></>}/>);
    return null};
