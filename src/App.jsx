...
export default function App() {
  ...
  const T=D.teams[team]||emptyTeam();
  ...
  const pgChat=()=>(<div className="cw"><div className="ph"><div className="pt">Chat</div></div>
    ...
    :<><div className="cmg">{T.chat.map(m=> <div className={`cbb ${m.from===me?'me':'ot'}`} key={m.id}><div className="cin">{m.from!==me&&<div className="cfr">{m.from}</div>}{m.text}<div className="cti">{ts(m.ts)}</div></div></div>)}<div ref={ce}/></div>
  ...
  const pgPo=()=>(<div><div className="ph"><div className="pt">Ankety</div><button className="ba" onClick={()=>setMod("aPo")}><Ic.Plus/></button></div>
    ...
    return (<div className="pc" key={p.id}><div className="pq">{p.question}</div>{p.options.map((o,i)=> <div key={i}><div className="pbw"><div className="pbl">{o.text}</div><div className="pbb"><div className="pbf" style={{width:Math.round(o.votes/tot*100)+"%",background:cl[i%5],color:'var(--bg)'}}>{o.votes}</div></div></div></div>)}<div style={{marginTop:5}}>{p.options.map((o,i)=> <button key={i} className="pvb" onClick={()=>vP(p.id,i)}>+ {o.text}</button>)}</div></div>)})}</div>);
  const pgPh=()=>(<div><div className="ph"><div className="pt">Fotky</div><button className="ba" onClick={()=>setMod("aPh")}><Ic.Plus/></button></div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:3,borderRadius:'var(--rd)',overflow:'hidden'}}>{T.photos.map((p,i)=> <div key={i} style={{aspectRatio:1,background:'var(--bg3)',overflow:'hidden'}}>{p.url&&<img src={p.url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>}</div>)}</div>}</div>);
  ...
  const modals=()=>{if(!mod) return null;
    ...
    if(mod==="aPh") return (<FM title="Přidat fotku" onS={f=>{const n=nt("Nová fotka","photos");saveT({...T,...n,photos:[...T.photos,{url:f.get('u'),caption:f.get('c'),date:td()}]});setMod(null)}} ch={<>{F("u","URL","url","https://...")}{F("c","Popis","text","",false)}<button type="submit" className="fs">Přidat</button></>}/>);
    return null};
