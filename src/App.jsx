...
export default function App() {
  ...
  const pgMatches=()=>{if(selM){const m=T.matches.find(x=>x.id===selM.id)||selM; return (<div>
    ...
    {showPitch&&(m.lineup||[]).length>0&&(()=>{const lp=T.players.filter(p=>(m.lineup||[]).includes(p.id));const pos=m.lineupPos||{};const pitchKey=(m.lineup||[]).sort().join(',');
      ...
      return <div className="lf" key={pitchKey}>{lp.map((p,i)=>{const{x,y}=defPos(p,i);return <div className="fp" key={p.id} style={{left:x+'%',top:y+'%',transform:'translate(-50%,-50%)'}} onMouseDown={e=>onDrag(p.id,e)} onTouchStart={e=>onDrag(p.id,e)}><div className="fc">{p.name.split(' ').map(w=>w[0]).join('')}</div><div className="fn">{pName(p)}</div></div>})}</div>})()}
