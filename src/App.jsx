...
export default function App() {
    ...
    <div style={{background:'var(--cd)',borderRadius:14,padding:12,boxShadow:'0 4px 14px rgba(0,0,0,.06),0 1px 3px rgba(0,0,0,.08),inset 0 1px 0 rgba(255,255,255,.9)',border:'1px solid rgba(22,163,74,.12)'}}>
      ...
      :<>{allDone.slice(0,5).map(d=> <div key={d.id} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 0',borderBottom:'1px solid var(--b)',cursor:'pointer'}} onClick={()=>{if(d._k==="match"){go("matches");setTimeout(()=>setSelM({id:d.id}),50)}else if(d._k==="train")go("trainings");else if(d._k==="meet"){go("meetings");setTimeout(()=>setSelMt(d),50)}else if(d._k==="vote"){go("votings");setTimeout(()=>setSelVt(d),50)}}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:11,fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d._k==="match"?<VS op={d.opponent} sz={11}/>:(d.focus||d.topic)}</div>
  ...
  const VS=({op,sz})=>{const s=sz||12;return <span style={{display:'inline-flex',alignItems:'center',gap:4}}><span style={{fontWeight:700,fontSize:s}}>DD</span><span style={{fontStyle:'italic',fontSize:s*.75,color:'var(--t3)',fontWeight:400,padding:'0 1px'}}>vs</span><span style={{fontWeight:700,fontSize:s}}>{op}</span></span>};
  ...
  const pgMatches=()=>{const selMatch=selM?T.matches.find(x=>x.id===(selM.id||selM)):null;if(selMatch){const m=selMatch; return (<div>
    ...
    <div className="pt" style={{marginBottom:8,opacity:m.done?.5:1}}><VS op={m.opponent} sz={18}/></div><div style={{color:'var(--t2)',fontSize:12,marginBottom:12}}>{fd(m.date)} · {m.time}{m.meetTime&&<span style={{color:'var(--ac)'}}> · sraz {m.meetTime}</span>} · {m.location==="Domácí"?"Doma":"Venku"} · {m.type}</div>
