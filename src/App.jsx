...
export default function App() {
  ...
  const [ci,setCi]=useState("");const ce=useRef(null);
  ...
  const addM=m=>{const n=nt("Zápas: "+m.opponent,"matches");saveT({...T,...n,matches:[...T.matches,{...m,id:"m_"+uid(),result:null,lineup:[],attendance:{}}]});setMod(null)};
  const addTr=t=>{const n=nt("Trénink: "+t.focus,"trainings");saveT({...T,...n,trainings:[...T.trainings,{...t,id:"t_"+uid(),attendance:{}}]});setMod(null)};
  ...
  const addMeet=m=>{const n=nt("Schůze: "+m.topic,"meetings");saveT({...T,...n,meetings:[{...m,id:"mt_"+uid(),attendance:{},docs:[]},...T.meetings]});setMod(null)};
  ...
  const openDoc=(doc)=>{const w=window.open('','_blank');if(w){w.document.write('<!DOCTYPE html><html><head><title>'+doc.name+'</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;font-family:sans-serif;background:#f0f0f0}.bar{padding:10px 16px;background:#fff;border-bottom:1px solid #ddd;display:flex;gap:12px;align-items:center;position:sticky;top:0;z-index:10}.bar a{color:#1a8aab;text-decoration:none;font-weight:600;font-size:14px}.cnt{padding:16px;text-align:center}</style></head><body><div class="bar"><a href="javascript:window.close()">← Zavřít</a><a href="'+doc.data+'" download="'+doc.name+'">⬇ Stáhnout</a><span style="color:#666;font-size:13px">'+doc.name+'</span></div><div class="cnt">'+(doc.data.startsWith('data:image')?'<img src="'+doc.data+'" style="max-width:100%;border-radius:8px"/>':'<iframe src="'+doc.data+'" style="width:100%;height:85vh;border:none;border-radius:8px"></iframe>')+'</div></body></html>');w.document.close()}};
  ...
  const pgMeetings=()=>{if(selMt){const m=T.meetings.find(x=>x.id===selMt.id)||selMt; return (<div>
    ...
    <div className="doc-list">{(m.docs||[]).map((d,i)=> <div key={i} style={{display:'flex',alignItems:'center',gap:4}}><button onClick={()=>openDoc(d)} className="doc-item" style={{cursor:'pointer',border:'1px solid var(--b)',background:'var(--cd)'}}><Ic.Doc/> {d.name}</button><a href={d.data} download={d.name} style={{fontSize:10,color:'var(--ac)',textDecoration:'none',padding:'4px 8px',background:'var(--ag)',borderRadius:12}}>⬇</a></div>)}</div>
  ...
  const pgPl=()=>(<div><div className="ph"><div className="pt">Hráči</div><button className="ba" onClick={()=>setMod("aP")}><Ic.Plus/></button></div>
    {T.players.sort((a,b)=>a.number-b.number).map(p=> <div className="pr" key={p.id} style={{flexWrap:'wrap'}}>
      <div className="pnn">{p.number}</div>
